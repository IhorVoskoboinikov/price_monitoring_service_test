...
- Пользователь регистрируется по email + пароль (auth выходит за рамки текущей версии).
...
Пользователь системы. Полноценный auth выходит за рамки текущей версии — используется статический JWT (см. раздел 12.4). Поле `password_hash` зарезервировано на будущее: в текущей версии регистрация/логин не реализованы, поэтому оно не заполняется.
...
> **Как пользователь формирует список отслеживания.** *Способ* создания
> списка намеренно оставлен открытым. Граница принята так: **каталог
...
> в списке (иначе 409), и добавляет строку; `user_id` берётся из JWT — список
> приватный. Страница списка товаров (4.1) и так возвращает **только** watchlist
> текущего пользователя (JOIN на `UserProduct`), а не весь каталог.
...
> production-мониторинга; в текущей версии не требуется.
...
> **Путь развития (вне рамок текущей версии):** если бы листался глобальный каталог
...
> **Путь развития (не реализовано, вне рамок текущей версии):** отдельная задача
...
> `ProductShop`. В текущей версии начальный маппинг делается общим `seed_products()`
...
**Стратегия в текущей версии — маппинг по близости цены при инициализации.**

Каталоги DummyJSON и FakeStore почти не пересекаются по товарам, поэтому
совмещать id «произвольным образом» допустимо. При первом запуске
...
>
> **Путь развития (вне рамок текущей версии):** кеш агрегированных цен
...
Целевой масштаб: товаров — до миллионов, история цен — за годы. Главная и самая
...
> скрипт покрывают требование «получать исторические курсы» без этой цены.
...
> **Почему без `exp`:** в текущей версии срок действия токена не нужен — токен генерируется один раз и используется для тестирования API. В production добавляется `exp` и refresh-механизм.
