...
| Индекс | Назначение |
|---|---|
| `PriceHistory(product_shop_id, recorded_at DESC)` | Основной запрос: история цен товара по времени |
| `PriceHistory(recorded_at)` | Ключ партиционирования |
| `ProductShop(product_id)` | JOIN при агрегации цен |
| `ExchangeRate(currency_code, date)` | UNIQUE — быстрый lookup курса |
| `PriceAlert(is_active, product_id)` | Фильтр активных алертов при проверке |
| `UserProduct(user_id)` | Список товаров пользователя |
...
- **Индекс на партиции:** каждая партиция имеет свой индекс по `(product_shop_id, recorded_at DESC)`.
